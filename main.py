...
def average_grade_student(list_student, course):
    res = sum([sum(student.grades[course]) for student in list_student]) / sum(
        [len(student.grades[course]) for student in list_student])
    print(res)


def average_grade_lecturer(list_lecturer, course):
    res = sum([sum(lecturer.grades[course]) for lecturer in list_lecturer]) / sum(
        [len(lecturer.grades[course]) for lecturer in list_lecturer])
    print(res)


reviewer_1 = Reviewer('Анна', 'Иванова')
reviewer_1.courses_attached.append('Ruby')
reviewer_1.courses_attached.append('C#')
reviewer_1.courses_attached.append('Python')

reviewer_2 = Reviewer('Сергей', 'Иванов')
reviewer_2.courses_attached.append('Python')
reviewer_2.courses_attached.append('Java')

lecturer_1 = Lecturer('Иван', 'Васильев')
